...
func responsesInputItemToChatMessages(item map[string]any, messages []dto.Message) ([]dto.Message, error) {
	...
	if !responsesInputItemHasChatMessageShape(itemType, item) {
		return messages, nil
	}
...
func responsesInputItemHasChatMessageShape(itemType string, item map[string]any) bool {
	if itemType != "" && itemType != responsesInputTypeMessage {
		return false
	}
	if itemType == responsesInputTypeMessage {
		return true
	}
	if strings.TrimSpace(common.Interface2String(item["role"])) != "" {
		return true
	}
...
func responsesContentPartsToChatContent(parts []any) (any, error) {
	...
	for _, rawPart := range parts {
		...
		case "input_text", "output_text", "text":
			...
			chatParts = append(chatParts, map[string]any{"type": dto.ContentTypeText, "text": text})
		case "input_image":
			onlyText = false
			chatParts = append(chatParts, map[string]any{"type": dto.ContentTypeImageURL, "image_url": responsesImagePartToChatImageURL(part)})
		case "input_file":
			onlyText = false
			chatParts = append(chatParts, map[string]any{"type": dto.ContentTypeFile, "file": responsesFilePartToChatFile(part)})
		case "input_audio":
			onlyText = false
			chatParts = append(chatParts, map[string]any{"type": dto.ContentTypeInputAudio, "input_audio": responsesPartPayload(part, "input_audio")})
		case "input_video":
			onlyText = false
			chatParts = append(chatParts, map[string]any{"type": dto.ContentTypeVideoUrl, "video_url": responsesVideoPartToChatVideoURL(part)})
		default:
			onlyText = false
