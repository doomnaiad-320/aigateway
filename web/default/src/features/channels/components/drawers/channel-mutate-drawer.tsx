...
export function ChannelMutateDrawer({
  ...
  const videoTaskPathOverrideEnabled = form.watch(
  ...
  const currentKey = formSnapshot.key
  const currentOther = formSnapshot.other
  const currentPriority = formSnapshot.priority
  const currentWeight = formSnapshot.weight
  const currentTestModel = formSnapshot.test_model
  const currentAutoBan = formSnapshot.auto_ban
  const currentStatusCodeMapping = formSnapshot.status_code_mapping
  const currentTag = formSnapshot.tag
  const currentRemark = formSnapshot.remark
  const currentParamOverride = formSnapshot.param_override
  const currentHeaderOverride = formSnapshot.header_override
  const currentForceFormat = formSnapshot.force_format
  const currentThinkingToContent = formSnapshot.thinking_to_content
  const currentProxy = formSnapshot.proxy
  const currentPassThroughBodyEnabled = formSnapshot.pass_through_body_enabled
  const currentSystemPrompt = formSnapshot.system_prompt
  const currentSystemPromptOverride = formSnapshot.system_prompt_override
  const currentAllowServiceTier = formSnapshot.allow_service_tier
  const currentDisableStore = formSnapshot.disable_store
  const currentAllowSafetyIdentifier = formSnapshot.allow_safety_identifier
  const currentAllowIncludeObfuscation = formSnapshot.allow_include_obfuscation
  const currentAllowInferenceGeo = formSnapshot.allow_inference_geo
  const currentAllowSpeed = formSnapshot.allow_speed
  const currentClaudeBetaQuery = formSnapshot.claude_beta_query
  const currentUpstreamModelUpdateAutoSyncEnabled =
    formSnapshot.upstream_model_update_auto_sync_enabled
  const currentUpstreamModelUpdateIgnoredModels =
    formSnapshot.upstream_model_update_ignored_models
  ...
  const apiAccessHasErrors = Boolean(
    formErrors.key ||
    formErrors.base_url ||
    formErrors.other ||
    ...
    formErrors.is_enterprise_account ||
    formErrors.advanced_custom
  )
  const modelsHaveErrors = Boolean(
    formErrors.models || formErrors.group || formErrors.model_mapping
  )
  const advancedHaveErrors = ADVANCED_ERROR_FIELDS.some((field) =>
    Boolean(formErrors[field])
  )
  const identityComplete = Boolean(currentName?.trim() && currentType > 0)
  const apiAccessComplete = Boolean(
    (isEditing || currentKey?.trim()) &&
    (!BASE_URL_REQUIRED_TYPES.has(currentType) || currentBaseUrl?.trim()) &&
    (!OTHER_REQUIRED_TYPES.has(currentType) || currentOther?.trim())
  )
  ...
  const apiAccessStatus = getCompletionStatus(
    apiAccessHasErrors,
    apiAccessComplete
  )
  ...
  const videoTaskProtocolConfigured = Boolean(
    isVideoTaskChannel &&
    (videoTaskPathOverrideEnabled || videoTaskProtocolEnabled)
  )
  ...
  if (isVideoTaskChannel) {
    advancedNavChildren.push({
      id: ADVANCED_SETTINGS_SECTION_IDS.videoTaskProtocol,
    ...
    if (videoTaskProtocolEnabled) {
      advancedNavChildren.push({
        ...
        configured: responseMappingConfigured,
      })
    }
  }
  ...
  const editorNavItems: ChannelEditorNavItem[] = [
    ...
    {
      id: CHANNEL_EDITOR_SECTION_IDS.apiAccess,
      title: t('API Access'),
      description: getSectionStatusLabel(apiAccessStatus, t),
      statusLabel: getSectionStatusLabel(apiAccessStatus, t),
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      toast.warning(
        t(
          'Warning: Base URL should not end with /v1. MAX API will handle it automatically. This may cause request failures.'
        ),
        { duration: 5000 }
      )
    }, 500)
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <FormDescription>
                                  {t(
                                    'Custom API base URL. For official channels, MAX API has built-in addresses. Only fill this for third-party proxy sites or special endpoints. Do not add /v1 or trailing slash.'
                                  )}
                                </FormDescription>
                                <FormMessage />
                              </FormItem>
                      ...
                      >
                        ...
                        <div className={sideDrawerSectionClassName()}>
                          ...
                          {isVideoTaskChannel && (
                            <div
                              id={
                                ADVANCED_SETTINGS_SECTION_IDS.videoTaskProtocol
                              }
                            ...
                            >
                              ...
                              <div className='divide-border space-y-0 divide-y border-y'>
                                <FormField
                                  control={form.control}
                                  name='video_task_path_override_enabled'
                                  render={({ field }) => (
                                    <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
                                      <div className='space-y-0.5'>
                                        <FormLabel>
                                          {t('Override video task paths')}
                                        </FormLabel>
                                        <FormDescription>
                                          {t(
                                            'Only override upstream submit/query paths and keep the official response parser'
                                          )}
                                        </FormDescription>
                                      </div>
                                      <FormControl>
                                ...
                                <FormField
                                  control={form.control}
                                  name='video_task_protocol_enabled'
                                  render={({ field }) => (
                                    <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
                                      <div className='space-y-0.5'>
                                        <FormLabel>
                                          {t('Enable video task protocol')}
                                        </FormLabel>
                                        <FormDescription>
                                          {t(
                                            'Use configurable third-party video task request and response mappings'
                                          )}
                                        </FormDescription>
                                      </div>
                                      <FormControl>
                              ...
                              {showVideoTaskPathFields && (
                                <div className='grid gap-4 sm:grid-cols-2'>
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        ...
                                        <FormControl>
                                          <Input
                                            placeholder='/v1/videos/create'
                                            {...field}
                                          />
                                        </FormControl>
                                        <FormDescription>
                                          {t('Upstream task creation path')}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                    )}
                                  ...
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        <FormLabel>{t('Query Path')}</FormLabel>
                                        <FormControl>
                                          <Input
                                        ...
                                        <FormDescription>
                                          {t(
                                            'Task query path, must include one of {task_id}, {operation_name}, or {upstream_task_id}'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                            ...
                            >
                              ...
                              <>
                                <div className='grid gap-4 sm:grid-cols-2'>
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        ...
                                        <FormDescription>
                                          {t(
                                            'Response path for upstream task ID'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                  ...
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        ...
                                        <FormDescription>
                                          {t('Response path for task status')}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                    )}
                                  ...
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        ...
                                        <FormDescription>
                                          {t('Response path for task progress')}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                    )}
                                  ...
                                  <FormField
                                    control={form.control}
                                    name='video_task_error_message_path'
                                    render={({ field }) => (
                                      <FormItem>
                                        <FormLabel>
                                          {t('Error Message Path')}
                                        </FormLabel>
                                        <FormControl>
                                          <Input
                                        ...
                                        <FormDescription>
                                          {t(
                                            'Response path for failure reason'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                ...
                                <FormField
                                  control={form.control}
                                  name='video_task_result_url_paths'
                                  render={({ field }) => (
                                    <FormItem>
                                      <FormLabel>
                                        {t('Result URL Paths')}
                                      ...
                                      <FormControl>
                                        <Textarea
                                          ...
                                          placeholder='result.url,result.video_url,url'
                                          {...field}
                                        />
                                      </FormControl>
                                      <FormDescription>
                                        {t(
                                          'Comma-separated response paths checked in order'
                                        )}
                                      </FormDescription>
                                      <FormMessage />
                                    </FormItem>
                                ...
                                <div className='grid gap-4 sm:grid-cols-2'>
                                  <FormField
                                    control={form.control}
                                    name='video_task_status_submitted'
                                    render={({ field }) => (
                                      <FormItem>
                                        <FormLabel>
                                          {t('Submitted Statuses')}
                                        </FormLabel>
                                        <FormControl>
                                          <Input
                                        ...
                                        <FormDescription>
                                          {t(
                                            'Comma-separated upstream statuses mapped to SUBMITTED'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                  ...
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        ...
                                        <FormControl>
                                          <Input
                                            placeholder='queued,pending'
                                            {...field}
                                          />
                                        </FormControl>
                                        <FormDescription>
                                          {t(
                                            'Comma-separated upstream statuses mapped to QUEUED'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                  ...
                                  <FormField
                                    control={form.control}
                                    name='video_task_status_running'
                                    render={({ field }) => (
                                      <FormItem>
                                        <FormLabel>
                                          {t('Running Statuses')}
                                        ...
                                        <FormControl>
                                          <Input
                                            placeholder='running,processing'
                                            {...field}
                                          />
                                        </FormControl>
                                        <FormDescription>
                                          {t(
                                            'Comma-separated upstream statuses mapped to IN_PROGRESS'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                  ...
                                  <FormField
                                    control={form.control}
                                    name='video_task_status_succeeded'
                                    render={({ field }) => (
                                      <FormItem>
                                        <FormLabel>
                                          {t('Succeeded Statuses')}
                                        </FormLabel>
                                        <FormControl>
                                          <Input
                                            placeholder='succeeded,success,completed'
                                            {...field}
                                          />
                                        </FormControl>
                                        <FormDescription>
                                          {t(
                                            'Comma-separated upstream statuses mapped to SUCCESS'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
                                  ...
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        ...
                                        <FormControl>
                                          <Input
                                            placeholder='failed,failure'
                                            {...field}
                                          />
                                        </FormControl>
                                        <FormDescription>
                                          {t(
                                            'Comma-separated upstream statuses mapped to FAILURE'
                                          )}
                                        </FormDescription>
                                        <FormMessage />
                                      </FormItem>
