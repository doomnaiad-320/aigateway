...
import (
	...
	"github.com/MAX-API-Next/MAX-API/router"
...
func main() {
	...
	common.SysLog("MAX API " + common.Version + " started")
	...
	server.Use(gin.CustomRecovery(func(c *gin.Context, err any) {
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": gin.H{
				"message": fmt.Sprintf("Panic detected, error: %v. Please submit a issue here: https://github.com/MAX-API-Next/MAX-API", err),
				"type":    "max_api_panic",
			},
		})
	}))
...
func InjectUmamiAnalytics() {
	...
	analyticsInjectBuilder.WriteString("<!--Umami MAX-API-Next-->\n")
...
func InjectGoogleAnalytics() {
	...
	analyticsInjectBuilder.WriteString("<!--Google Analytics MAX-API-Next-->\n")
	analyticsInject := []byte(analyticsInjectBuilder.String())
