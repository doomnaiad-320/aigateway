...
import (
	...
	"github.com/MAX-API-Next/MAX-API/setting/console_setting"
...
func UpdateOption(c *gin.Context) {
	...
	case "theme.frontend":
		if option.Value != "default" {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "无效的主题值，可选值：default（默认前端）",
			})
			return
		}
	case "GroupRatio":
	...
	case "task_billing_setting.rate_cards":
		err = task_billing_setting.ValidateRateCardsJSON(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
