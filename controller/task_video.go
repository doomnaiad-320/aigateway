...
func updateVideoSingleTask(ctx context.Context, adaptor channel.TaskAdaptor, channel *model.Channel, taskId string, taskM map[string]*model.Task) error {
	...
	resp, err := adaptor.FetchTask(baseURL, key, taskcommon.WithTaskProtocolConfig(map[string]any{
		"task_id": taskId,
		"action":  task.Action,
		"model":   task.Properties.UpstreamModelName,
	}, channel.GetOtherSettings()), proxy)
	if err != nil {
	...
	if err = common.Unmarshal(responseBody, &responseItems); err == nil && responseItems.IsSuccess() {
		...
		task.Data = t.Data
	} else if configuredResult, ok, parseErr := taskcommon.ParseConfiguredTaskResult(responseBody, channel.GetOtherSettings()); parseErr != nil {
