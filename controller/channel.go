...
import (
	...
	relaychannel "github.com/MAX-API-Next/MAX-API/relay/channel"
...
func AddChannel(c *gin.Context) {
	...
	channels, channelNames := buildChannelsForInsert(addChannelRequest.Channel, keys, addChannelRequest.BatchAddSetKeyPrefix2Name)
...
func buildChannelsForInsert(baseChannel *model.Channel, keys []string, setKeyPrefixToName bool) ([]model.Channel, []string) {
	channels := make([]model.Channel, 0, len(keys))
	channelNames := make([]string, 0, len(keys))
	for _, key := range keys {
		...
		if setKeyPrefixToName && len(keys) > 1 {
		...
		channelNames = append(channelNames, localChannel.Name)
	}
...
func FetchModels(c *gin.Context) {
	...
	if err := common.ValidateURLWithFetchSetting(url, fetchSetting.EnableSSRFProtection, fetchSetting.AllowPrivateIp, fetchSetting.DomainFilterMode, fetchSetting.IpFilterMode, fetchSetting.DomainList, fetchSetting.IpList, fetchSetting.AllowedPorts, fetchSetting.ApplyIPFilterForDomain); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": fmt.Sprintf("request reject: %v", err),
		})
		return
	}
...
func ManageMultiKeys(c *gin.Context) {
	...
	recordMultiKeyManageAudit := func() {
