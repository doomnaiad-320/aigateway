...
func geminiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response, callback func(data string, geminiResponse *dto.GeminiChatResponse) bool) (*dto.Usage, *types.MaxAPIError) {
	var usage = &dto.Usage{}
	var imageCount int
	...
	helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
		...
		if service.ResponseAuditEnabled() {
			if text := service.BuildGeminiNonTextResponseAuditContent(&geminiResponse); text != "" {
				if auditText.Len() > 0 {
					auditText.WriteByte('\n')
				}
				auditText.WriteString(text)
			}
		}
	...
	if service.ResponseAuditEnabled() {
		auditContent := responseText.String()
		if extra := strings.TrimSpace(auditText.String()); extra != "" {
		...
		service.SetRelayResponseAuditContent(info, auditContent)
	}
	return usage, nil
}

func GeminiChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
	id := helper.GetResponseID(c)
...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
	...
	if len(geminiResponse.Candidates) == 0 {
		...
		if geminiResponse.PromptFeedback != nil && geminiResponse.PromptFeedback.BlockReason != nil {
			common.SetContextKey(c, constant.ContextKeyAdminRejectReason, fmt.Sprintf("gemini_block_reason=%s", *geminiResponse.PromptFeedback.BlockReason))
			maxAPIError = types.NewOpenAIError(
...
func GeminiEmbeddingHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
...
func GeminiImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
	responseBody, readErr := io.ReadAll(resp.Body)
	if readErr != nil {
		return nil, types.NewOpenAIError(readErr, types.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}
	...
	if service.ResponseAuditEnabled() {
		service.SetRelayResponseAuditContent(info, service.BuildSimpleImageAuditContent(generatedImages))
	}
