...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.MaxAPIError) {
	...
	switch converter := a.converter(); converter {
	case dto.AdvancedCustomConverterNone:
	...
	default:
		return nil, types.NewOpenAIError(fmt.Errorf("unsupported advanced custom converter: %s", converter), types.ErrorCodeInvalidRequest, http.StatusBadRequest, types.ErrOptionWithSkipRetry())
	}
}
...
func (a *Adaptor) doNativeResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (any, *types.MaxAPIError) {
...
func (a *Adaptor) converter() string {
	converter := strings.TrimSpace(a.route.Converter)
	if converter == "" {
		return dto.AdvancedCustomConverterNone
	}
	return converter
}
...
func joinBaseURLAndUpstreamPath(baseURL string, upstreamPath string) (*url.URL, error) {
	...
	basePath := parsedBaseURL.EscapedPath()
	upstreamEscapedPath := parsedPath.EscapedPath()
	joinedEscapedPath := strings.TrimRight(basePath, "/") + "/" + strings.TrimLeft(upstreamEscapedPath, "/")
	joinedPath, err := url.PathUnescape(joinedEscapedPath)
	if err != nil {
		return nil, err
	}
	parsedBaseURL.Path = joinedPath
	parsedBaseURL.RawPath = joinedEscapedPath
...
func applyUpstreamPathTemplate(upstreamPath string, info *relaycommon.RelayInfo) string {
	...
	return strings.ReplaceAll(upstreamPath, advancedCustomModelPlaceholder, url.PathEscape(info.UpstreamModelName))
}
...
func shouldUseGeminiStreamURL(converter string, info *relaycommon.RelayInfo) bool {
	return info != nil &&
		...
		(converter == dto.AdvancedCustomConverterOpenAIChatCompletionsToGeminiGenerateContent ||
			(converter == dto.AdvancedCustomConverterNone && info.RelayFormat == types.RelayFormatGemini))
}
...
func withOpenAIChannelType[T any](info *relaycommon.RelayInfo, convert func() (T, error)) (T, error) {
...
func (a *Adaptor) convertOpenAICompatibleRequest(c *gin.Context, info *relaycommon.RelayInfo, request *dto.GeneralOpenAIRequest) (any, error) {
	return withOpenAIChannelType(info, func() (any, error) {
...
func (a *Adaptor) convertOpenAICompatibleAudioRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.AudioRequest) (io.Reader, error) {
	return withOpenAIChannelType(info, func() (io.Reader, error) {
