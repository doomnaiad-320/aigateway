						...
						if !strings.HasPrefix(file.FileData, "http://") && !strings.HasPrefix(file.FileData, "https://") {
							...
							if strings.HasPrefix(fileData, "data:") {
								if idx := strings.Index(fileData, ","); idx != -1 {
									mimeType = strings.Split(fileData[5:idx], ";")[0]
									fileData = fileData[idx+1:]
								}
							}
							if mimeType == "" {
								if dot := strings.LastIndex(file.FileName, "."); dot != -1 && dot+1 < len(file.FileName) {
									if mt := service.GetMimeTypeByExtension(file.FileName[dot+1:]); mt != "application/octet-stream" {
										mimeType = mt
									}
								}
							}
					...
					for _, toolCall := range message.ParseToolCalls() {
						...
						if args := toolCall.Function.Arguments; args != "" {
							if err := common.Unmarshal([]byte(args), &inputObj); err != nil {
								return nil, fmt.Errorf("tool call function arguments is not valid JSON object: %w", err)
							}
						}
						claudeMediaMessages = append(claudeMediaMessages, dto.ClaudeMediaMessage{
							Type:  "tool_use",
...
func HandleStreamResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, data string) *types.MaxAPIError {
...
func HandleClaudeResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, httpResp *http.Response, data []byte) *types.MaxAPIError {
	var claudeResponse dto.ClaudeResponse
	...
	case types.RelayFormatOpenAI:
		...
		responseData, err = json.Marshal(auditResponse)
		if err != nil {
			return types.NewError(err, types.ErrorCodeBadResponseBody)
		}
	...
	if service.ResponseAuditEnabled() {
		service.SetRelayResponseAuditContent(info, service.BuildTextResponseAuditContent(auditResponse))
	}
