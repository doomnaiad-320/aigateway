...
func sendStreamData(c *gin.Context, info *relaycommon.RelayInfo, data string, forceFormat bool, thinkToContent bool) error {
	...
	maskedData, err := maskChatStreamResponseModel(info, data)
	if err != nil {
		return err
	}
	data = maskedData
...
func OaiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
...
func OpenaiHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
	defer service.CloseResponseBodyGracefully(resp)
	...
	if modelName := clientVisibleModelName(info); modelName != "" {
		simpleResponse.Model = modelName
	}
...
func OpenaiRealtimeHandler(c *gin.Context, info *relaycommon.RelayInfo) (*types.MaxAPIError, *dto.RealtimeUsage) {
...
func OpenaiHandlerWithUsage(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
	defer service.CloseResponseBodyGracefully(resp)
	...
	if service.ResponseAuditEnabled() {
		...
		if err := common.Unmarshal(responseBody, &textResp); err == nil {
			service.SetRelayResponseAuditContent(info, service.BuildTextResponseAuditContent(&textResp))
		}
	}
