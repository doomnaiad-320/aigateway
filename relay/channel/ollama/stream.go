...
func ollamaToolCallsToOpenAI(toolCalls []OllamaToolCall, startIndex int, includeIndex bool) ([]dto.ToolCallResponse, int) {
	...
	for _, tc := range toolCalls {
		...
		if tc.Function.Arguments != nil {
			if data, err := common.Marshal(tc.Function.Arguments); err == nil && len(data) > 0 {
				argBytes = data
			}
		}
...
func ollamaStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
...
func ollamaChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.MaxAPIError) {
